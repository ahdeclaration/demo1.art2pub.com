...
if(!empty($showthisadinSidebar)){
	...
	if(has_post_thumbnail()) {
        ...
        if( $typeofcampaign == 'adsperclick' && $listindUserID != $current_id ){
            if(is_front_page() && $front_page_adStatus == 'active'){
                $ad_listing_url = esc_url(add_query_arg (array( 'ad_type' => 'lp_random_ads_pc' ), get_the_permalink()));
            }else if( ((isset($_GET['s']) && $_GET['s'] == 'home') || is_archive()) && $search_page_adStatus == 'active'){
                $ad_listing_url = esc_url(add_query_arg (array( 'ad_type' => 'lp_top_in_search_page_ads_pc' ), get_the_permalink()));
            }elseif(is_single() && $detail_page_adStatus == 'active'){
